...
#### Send Message
- **Endpoint**: `POST /message/send/{reciver_id}`
- **Description**: Send a message to a user.
- **Headers Required**: 
  - `Authorization: Bearer <access_token>`
- **Parameters**:
  - `reciver_id` (path parameter, required) - The ID of the user to send the message to
  - `message` (body parameter, required) - The message to send
  - `job_id` (body parameter, required) - The ID of the job
  - `type` (body parameter, required) - The type of the message
  - `sender_id` (body parameter, required) - The ID of the sender

- **Sample Request (curl)**:

  ```bash
  curl -X POST "http://your-api-url/message/send/1" \
    -H "Authorization: Bearer <access_token>" \
    -d "message=Hello, how are you?" \
    -d "job_id=16" \
    -d "type=text" \
    -d "sender_id=f9b86db6-93dc-4e29-a1be-6dfcb114b8f7"
  ```
...
- **Sample Response**:
  ...
  ```json
    ...
    "data": {
      ...
      "message": "Testng Realtime",
      "type": "text",
      "created_at": "2025-07-17T13:22:57.923599+00:00",
      "is_read": false
    }
    }
---
